...
def test_closed_politics_candidates_preserve_occurrence_blocker_and_choose_one_binary_market():
    events = [
        {
            "id": "42",
            "title": "Will the Senate vote pass?",
            ...
            "markets": [
                ...
                },
                {
                    ...
                    "volumeNum": 2,
                    "closedTime": "2026-08-02T00:00:00Z",
                },
            ],
        }
    ]
    ...
    candidates = select_archive_candidates(
        events, max_events=20, archive_cutoff=ARCHIVE_CUTOFF
    )
    ...
    assert (
        candidates[0]["archive_provenance"]["event_end_date"] == "2026-08-01T00:00:00Z"
    )
...
def test_candidates_reject_nonbinary_and_unrelated_archive_events():
    events = [
        {"id": "bad", "title": "Will Bitcoin close above $100,000?", "markets": []},
        {
            ...
            "title": "Election vote",
            "markets": [
                {"closed": True, "outcomes": '["A", "B"]', "clobTokenIds": '["a", "b"]'}
            ],
        },
    ]

    assert (
        select_archive_candidates(events, max_events=5, archive_cutoff=ARCHIVE_CUTOFF)
        == []
    )
    ...
    events = [
        {
            ...
            "endDate": "2027-11-04T00:00:00Z",
            "markets": [
                {
                    "id": "prematurely-closed",
                    "closed": True,
...
def test_external_occurrence_ledger_requires_exact_candidate_and_preserves_unmatched_blocker():
    candidates = [
        ...
        {
            "event_id": "polymarket-event-vote",
            "occurrence_at": None,
            "occurrence_status": "requires_authoritative_event_source",
        },
    ]
    ...
    assert (
        enriched[0]["occurrence_provenance"]["evidence_type"] == "exact_event_id_ledger"
    )
...
def test_external_occurrence_ledger_rejects_duplicate_or_non_https_evidence():
    ...
    duplicate = [
        ...
        {
            "event_id": "polymarket-event-speech",
            "occurrence_at": "2026-07-31T19:00:00Z",
            "source_name": "official",
    ...
    with pytest.raises(ValueError, match="HTTPS"):
        attach_authoritative_occurrences(
            [candidate],
            [
                {
                    "event_id": "polymarket-event-speech",
