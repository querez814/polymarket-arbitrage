...
| G10 | **PARTIAL** | `kalshi_client/models.py:52-125` correctly derives complete YES/NO ask depth from the opposite bid book, and `kalshi_client/api.py:476-537` reads and sorts current `orderbook_fp` levels. This supersedes the source report's unsupported arbitrary 2% haircut. | Parsing converts authoritative fixed-point strings to binary floats, accepts the deprecated shape alongside V2 without a fail-closed schema boundary, and lacks focused depth/empty-side/malformed-level tests and freshness enforcement. Live depth quality is unproved. |
