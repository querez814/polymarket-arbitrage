...
| G4 | **PARTIAL** | `utils/config_loader.py:493-561` fails closed on live mode, simulation flags, current production hosts/chains, credential fragments, and wallet-key source; `tests/test_config_loader.py` proves the legacy Kalshi elections root is rejected in favor of the documented `external-api.kalshi.com` root, while the redacted Keychain-backed probe verifies the offline wallet boundary. | Tracked-versus-ignored credential-source policy and a final conservative-default audit remain; Kalshi credential validity cannot be proved without prohibited authenticated access. |
...
- G4 remains open: the current Kalshi production root is now consistent and fail-closed, but tracked-versus-ignored credential-source enforcement and conservative production defaults have not yet been fully reconciled. Actual Kalshi authenticated credential validation remains part of G2 because no Kalshi order lifecycle is implemented.
