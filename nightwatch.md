...
| G4 | **PARTIAL** | `utils/config_loader.py` fails closed on live mode, simulation flags, current production hosts/chains, credential fragments, wallet-key source, and secret values embedded in Git-tracked live YAML; `tests/test_config_loader.py` verifies tracked-file rejection, environment-injected acceptance, and rejection of the legacy Kalshi elections root, while the redacted Keychain-backed probe verifies the offline wallet boundary. | A final conservative-default audit remains; Kalshi credential validity cannot be proved without prohibited authenticated access. |
...
- G4 remains open: the current Kalshi production root and tracked-versus-ignored secret-source policy are now consistent and fail-closed, but conservative production defaults have not yet been fully reconciled. Actual Kalshi authenticated credential validation remains part of G2 because no Kalshi order lifecycle is implemented.
