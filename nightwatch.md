...
Read-only production credential validation was repeated on 2026-07-21 EDT after the Kalshi V2 contract work:

- Kalshi authenticated `GET /portfolio/balance` and `GET /portfolio/positions` succeeded using the API key ID and private-key file referenced by the local `config.yaml`. No credential value, signature, raw response, or balance amount was printed.
- Polymarket authenticated balance and open-order reads succeeded using the ignored `config.polymarket.yaml` L2 fields plus the `polymarket-trading-wallet` Keychain item. The response contained zero open orders; no credential or balance amount was printed.
- The reusable validator is `scripts/validate_readonly_credentials.py`. It contains no placement or cancellation call and reports sanitized status metadata only.
- This proves both configured credential pairs can authenticate read-only production account calls. It does not prove mutation permissions, order lifecycle correctness, funding adequacy, startup orchestration, profitability, or production readiness.
...
Audited through 2026-07-22 EDT. **RESOLVED** means both the required code and admissible evidence are complete; **PARTIAL** means a tested foundation exists but an integration or evidence requirement remains; **BLOCKED** means the acceptance path is absent or cannot be proved inside this run's safety boundary. Code completeness and prohibited live evidence are stated separately so an offline pass is never represented as production proof.

| ID | Status | Current code and offline evidence | Missing proof or implementation |
|---|---|---|---|
| G1 | **PARTIAL** | The production Polymarket V2 adapter is integrated with the locked executor and carries precomputed, journaled order hashes through placement, lookup, open-order reads, positions, and cancellation. | No authorized mutation lifecycle has proved actual venue fill, cancellation, funding, or signature behavior. |
| G2 | **PARTIAL** | The production Kalshi V2 adapter and authenticated private lifecycle stream are integrated into the production runtime, including stable IDs, strict fixed point, bounded dedupe, reconnect, and subscribe-before-REST reconciliation. | No authorized mutation lifecycle or live private-stream reconnect has been exercised. |
| G3 | **PARTIAL** | `run_with_dashboard.py` routes live cross-platform opportunities through the sole `ProductionArbitrageRuntime`, startup recovery, authoritative economics, and `LockedArbitrageExecutor`. The scarce-leg-first deterministic failure matrix passes offline. | No controlled venue mutation has proved fill/cancel latency, race behavior, or staffed residual response. No claim of atomic execution is made. |
| G4 | **PARTIAL** | `utils/config_loader.py` fails closed on live mode, simulation flags, production hosts/chains, credentials, wallet-key source, tracked secrets, required operator/alert controls, positive cross-platform exposure, and strategy paths that bypass recovery. Both configured venue credential pairs passed sanitized read-only calls. | Full supervised live startup remains unproved and execution stays disabled in tracked templates. Credential validity is no longer the blocker. |
| G5 | **PARTIAL** | The locked runtime enforces per-order, cumulative strategy/global/per-market/position-count limits, requires explicit two-venue whitelisting and authoritative collateral on both venues, and atomically reserves two possible attempts in a durable SQLite rolling/daily budget. | Live cap, collateral, and contention behavior under the intended single-owner deployment remain unproved. |
| G6 | **PARTIAL** | `AuthoritativeEconomicsProvider` consumes current pair-bound Polymarket V2 fee curves and Kalshi event/series fee schedules, models off-chain CLOB order gas explicitly as zero, applies documented upward precision, and rejects stale, missing, inconsistent, or unsupported metadata. | A controlled fill must verify actual debited fees and precision against the model; the linked Kalshi schedule PDF was still unavailable in this environment. |
| G7 | **PARTIAL** | Production venue readers, immutable journal admission, startup recovery, per-plan account-flat recovery, private-event reconciliation, and durable residual halting are wired into the production runtime. | Live restart/missed-event recovery and operator handling have not been exercised against venue state. |
| G8 | **PARTIAL** | Critical startup, panic, resume, stream-failure, execution-failure, and residual events use a bounded HTTPS webhook. Resume fails closed when delivery fails, and offline delivery/failure tests pass. | Production destination routing, retry/escalation policy, and staffed receipt have not been proved. |
| G9 | **PARTIAL** | Matching remains visible and replay-tested, while live execution now requires both the Polymarket condition ID and Kalshi ticker in an explicit configuration whitelist; canary preflight requires exactly one pair. | No evidence-backed representative false-positive audit or production match-quality rate exists. Automatic matches may reach read-only detection but cannot reach execution without explicit pair approval. |
...
| G11 | **PARTIAL** | Bearer-authenticated status/panic/resume endpoints control a durable SQLite halt checked at runtime admission. Every process restart requires explicit re-arming, live routes bind loopback, and alert failure blocks resume. | TLS-proxied production access, operator drill, and cross-process deployment behavior remain unproved. |
| G12 | **PARTIAL** | Python 3.12 dependencies are version-and-hash locked; public liveness and critical-service/trading readiness probes exist; the hardened systemd unit enforces one unprivileged owner, secret/state ownership, preflight, restricted writes, SIGTERM, and bounded restart; a local dashboard loopback start/SIGTERM/restart probe passed. Deployment and canary runbooks are committed. | The target Linux host must still pass `systemd-analyze`, the real entrypoint/service restart, durable backup/restore, TLS proxy, log retention, clock, and real alert receipt. The local probe is not full service-envelope proof. |
...
Matrix result: **0 RESOLVED, 14 PARTIAL, 0 BLOCKED**. Every code-addressable gate now has a tested foundation, but the evidence-based verdict remains **NOT PRODUCTION AUTHORIZED** until the target deployment, real alert route, private stream, and one minimum-size exchange lifecycle pass their staffed gates.
...
### 2026-07-21 — Kalshi V2 contract and credential checkpoint

- Browserbase resolved Kalshi's official Predictions REST `openapi.yaml` and Predictions WebSocket `asyncapi.yaml` download assets from `https://docs.kalshi.com/welcome`. The equivalent `kalshi.com` terminal URLs returned HTTP 429, so the official `docs.kalshi.com` mirrors were downloaded, YAML-validated, checksummed, and pinned under `specs/kalshi/` with provenance in `specs/kalshi/README.md`.
- Added strict V2 order templates and typed responses in `kalshi_client/orders.py`, covering fixed-point contract counts and prices, canonical `bid`/`ask`, time-in-force, self-trade prevention, stable client intent IDs, partial fills, remaining size, fees, cancellation reductions, and canonical order status/direction.
- Added current V2 create and cancel mutations plus authoritative get/list reconciliation reads in `kalshi_client/api.py`. Mutation transport is deliberately single-attempt: any request-level timeout or disconnect raises `KalshiMutationAmbiguousError` and requires authoritative reconciliation before retry.
- Added `tests/test_kalshi_order_v2.py`, which contracts the pinned OpenAPI/AsyncAPI paths and channels and verifies strict serialization, dry-run rejection, partial-fill parsing, exactly-one mutation attempt, URL-safe cancellation, and typed order reconciliation.
- `uv run --with-requirements requirements.txt python -m pytest -q`: **PASS — 287 tests** with 425 pre-existing deprecation warnings.
- Focused Kalshi suite: **PASS — 30 tests**. Focused Black: **PASS**. Changed-file mypy over `kalshi_client/orders.py`, `kalshi_client/api.py`, and `scripts/validate_readonly_credentials.py`: **PASS**.
- `scripts/validate_readonly_credentials.py` performed only authenticated read operations. Kalshi balance/positions and Polymarket balance/open orders both passed without printing credential values, signatures, raw responses, or balances. No order was constructed for submission, placed, cancelled, or simulated as submitted.

### 2026-07-21 — Locked-arbitrage execution gate (items 1-3)

- Recorded the approved execution policy in `future-convergence-strategy.md`: production is locked arbitrage only. The scarce/less-liquid leg uses IOC behavior; the hedge is sized only from the first leg's authoritative fill; ambiguity stops execution and requires reconciliation. Directional convergence trading is explicitly deferred as a separate future strategy with its own research queue and production gate.
- Added `core/venue_execution.py` with `LockedArbitrageExecutor` and a narrow venue mutation contract. Every state change is journaled before the next possible mutation. An unexpected open IOC is cancelled before hedging, a zero first-leg fill skips the unsubmitted hedge, and ambiguous submit/cancel outcomes are never retried.
- Extended the journal/state model with a durable precomputed venue order ID and an explicit unsubmitted `SKIPPED` terminal phase. Polymarket V2 orders are signed and hashed before POST, then that hash is journaled to close the request/response crash window.
- Added production `KalshiVenueAdapter` and `PolymarketVenueAdapter` implementations for placement, cancellation, authoritative order reads, account-wide open-order reads, and signed normalized positions. Polymarket normalized SELL exposure is implemented by buying the complementary NO token rather than assuming naked-short inventory.
- Added `kalshi_client/private_stream.py` for authenticated `fill` and `user_orders` messages, strict fixed-point parsing, bounded event deduplication, reconnect backoff, and subscribe-before-REST reconciliation so messages arriving during the snapshot are queued by the socket.
- Added a deterministic failure matrix covering zero, partial, and full first-leg fills; exact partial hedging; second-leg partial/rejected/ambiguous outcomes; unexpected resting IOC cancellation; ambiguous cancellation; restart durability; and residual exposure.
- Pinned `py-clob-client-v2>=1.0.2`, the locally verified installed SDK version that exposes V2 signed metadata/order hashing and includes the relevant precision fixes.
- Focused execution/recovery/adapter/stream suite: **PASS — 62 tests**. Complete discovered offline suite: **PASS — 305 tests** with 423 pre-existing deprecation warnings. Changed-source mypy with skipped external imports: **PASS — 11 files**. The broader mypy run still reaches pre-existing missing stubs/type debt outside this slice.
- No bot, dashboard, scanner, collector, live WebSocket, authenticated mutation, order submission, cancellation, simulated submission, or account mutation was started or performed. Read-only auth evidence remains from the earlier sanitized checkpoint.

### 2026-07-22 — Runtime, economics, and operations gate (items 4-6)

- Added `ProductionArbitrageRuntime` as the sole live cross-platform mutation owner. The dashboard entrypoint starts with a durable halt, establishes account-wide recovery, owns the Kalshi private-stream callback/reconciliation lifecycle, and routes detected opportunities through current economics and the locked executor.
- Added current pair-bound economics. Polymarket V2 market metadata supplies the price-curve rate/exponent and is cross-checked against its taker base fee; Kalshi event/series fee metadata supplies the supported quadratic multiplier. Stale, inconsistent, unavailable, maker-fee, or unsupported fee data fails closed.
- Added durable conservative order-attempt reservations; explicit two-venue whitelisting; cumulative strategy/global/per-market/position-count limits; authoritative collateral checks on both venues; shared 0.01 contract precision; fresh account reconciliation; and a post-recovery freshness check immediately before journal admission.
- Corrected the Gamma-market/CLOB-condition identity boundary so fee lookup, token resolution, order identity, positions, and restart recovery use the Polymarket condition ID while data-feed lookups retain the Gamma ID. Private-stream disconnect now durably halts admission, and residual/recovery-required executions are recorded as operator errors rather than successful trades.
- Added an owner-only persistent operator halt, mandatory re-arm after every restart, authenticated status/panic/resume endpoints, bounded HTTPS alerts, fail-closed resume when alert delivery fails, residual escalation, and loopback-only dashboard binding in live mode.
- Added `docs/production-operations.md` with startup, supervisor, storage, TLS proxy, panic/resume, incident recovery, deployment, and separately authorized canary gates. Live templates keep execution disabled and document environment-only control secrets.
- Complete discovered offline suite: **PASS — 336 tests**. Focused mypy over ten production modules, Python compilation, and `git diff --check`: **PASS**. No bot, dashboard, scanner, private WebSocket, authenticated venue request, order construction/submission/cancellation, simulated submission, account mutation, or live canary was run.

### 2026-07-22 — Deployment envelope and canary preparation

- Added `/health/live` for supervisor liveness and `/health/ready` for bot/trading admission. A healthy operator-halted process stays live while readiness fails closed; systemd never treats a deliberate trading halt as a crash.
- Added side-effect-free deployment/canary preflight with named JSON checks for live-real mode, locked strategy scope, Polymarket Global/Kalshi wiring, owner-private durable state/key destinations, root-owned private environment secrets, independent operator/alert secrets, authenticated HTTPS alerts, bounded exposure, one explicit pair, a two-attempt budget, and exact separately approved canary size/notional ceilings. Ordinary deployment now rejects execution-enabled configuration.
- Added a Python 3.12 version-and-hash `requirements.lock`, release-local environment verification, and a hardened single-owner `deploy/systemd/nightwatch.service` with same-config preflight/start, loopback liveness, SIGTERM, bounded restart, unprivileged execution, and filesystem restrictions. Critical matcher/scanner loss now fails readiness and triggers durable panic/escalation.
- Added `docs/canary-protocol.md` with the exact no-go checks, staffed authorization boundary, evidence bundle, one-plan window, mandatory final panic, and promotion boundary. ML remains a parallel research path and is not invoked by the deterministic locked-arbitrage runtime.
- Verified the bare dashboard probe's loopback start, liveness, SIGTERM, port release, and restart in a separate unsandboxed local test: **PASS — 1 test**. This is a probe lifecycle test, not a full entrypoint/systemd drill. The normal complete suite is **PASS — 358 tests, 1 loopback test skipped by sandbox policy**. Focused mypy for the five new preflight/probe/operator modules, Python compilation, new-file Black, and `git diff --check`: **PASS**. The broader legacy dashboard/entrypoint mypy baseline remains 27 errors in two files.
- No exchange, private venue stream, alert webhook, production host, credentials, order construction/submission/cancellation, simulated submission, or account mutation was accessed.

### 2026-07-22 — Honest $1,000 paper-performance correction

- Replaced the former random-fill/naked-sell paper behavior with a conservative locked-arbitrage paper ledger. Capital is reserved for both legs, projected PnL is settlement-based, collateral cannot be recycled early, and only an explicitly reviewed pair can record a shadow fill.
- Corrected Kalshi discovery to exclude multivariate-event combinations and to consume ordinary markets through `/events` with nested market and parent-event context. Discovery now remains alive and refreshes every 300 seconds; a zero-match cycle is an honest `no_matches` wait state rather than a false price-scan or critical-task failure.
- Added indexed candidate generation and cooperative matching. The observed run compared 177,040 meaningful candidates rather than 20,665,336 category-wide combinations while keeping 66 subthreshold candidates visible for human review and non-tradable.
- A real-public-data paper run loaded 10,000 eligible Kalshi markets and 2,074 Polymarket markets. Multiple completed refreshes found zero approved equivalent pairs, recorded zero trades, and left available and projected settlement equity exactly at the initial $1,000. The review candidates included materially different propositions such as election winner versus runoff qualification, validating the decision not to lower the automatic threshold.
- The final security review also closed two dashboard-only findings: `/ws` now rejects missing/cross-site origins and caps concurrent connections, while venue-controlled market text is HTML-escaped before insertion into dashboard cards.
...
- G1-G8/G11 now have the runtime, economics, controls, alerting, durable admission, adapters, locked executor, and private-stream implementation required for an offline production candidate. They remain partial because no separately authorized exchange mutation/private-stream lifecycle, deployed alert route, or operator drill has proved venue and incident behavior.
- G14's data/model path remains one-class and cannot make a quality or profitability claim. It is a parallel research gate, not an execution dependency: the production locked-arbitrage runtime never loads or calls ML inference.
- G4's configured production credentials are authenticated read-only for both venues. The execution flag remains disabled in live templates; full supervised startup and a staffed canary are unproved.
- G12's deployable package is complete offline, including hash-locked dependencies, environment verification, probes, preflight, supervisor definition, and graceful shutdown logic. Only the bare dashboard probe has completed a local process restart; the real entrypoint/systemd restart, target-host backup/restore, TLS, clock, log retention, and alert receipt remain external.
- The authoritative Kalshi fee-schedule PDF remained blocked by an external HTTP 429 challenge. Runtime now consumes current event/series metadata, but a controlled fill must still validate actual debits and rounding.
- The Python 3.12 dependency graph is version-and-hash locked in `requirements.lock`; the target Linux host must still build the release-local environment and pass startup verification.
- Focused static verification passes for the new runtime/economics/operations modules. The broader legacy entrypoint/dashboard mypy baseline still reports 27 errors in two files, so repository-wide static cleanliness remains outstanding; the complete discovered test suite and Python compilation pass.
...
The branch is now a code-complete pre-production candidate for the deterministic locked-arbitrage scope: the full sandbox suite passes 358 tests, the separate dashboard loopback probe/restart test passes, dependencies are version-and-hash locked for Python 3.12, deployment/preflight/probe/canary artifacts exist, and every runtime mutation gate still fails closed. It is not production-authorized because the actual entrypoint/systemd target-host drill, external alert route, live private-stream lifecycle, and one controlled minimum-size exchange lifecycle have not been exercised. The incomplete ML dataset does not block this deterministic runtime because no production path invokes it.

The next gate is entirely operational: deploy commit-pinned code with execution disabled, pass the target-host checklist and real alert drill, then separately authorize the one-plan canary in `docs/canary-protocol.md`. Credentials, preflight, and offline tests are not mutation authorization.
