...
- G1–G2 and G6–G13 have not yet been fully audited against current code or current official protocol behavior. G14's code-fixable dataset, training/evaluation, artifact, and subordinate-inference pipeline is implemented, but sufficiently diverse real matched data remains an external blocker to a fitted model and honest out-of-sample quality evaluation. G3 now fails closed instead of blindly retrying ambiguous placement errors, live cancellation retains residual exposure until one terminal refresh, live startup requires readable flat venue state before creating execution tasks, and an offline two-leg contract models stable intent-level keys plus realized/potential residual exposure. It still lacks persisted and venue-connected idempotency keys, an authoritative implemented position reader, a crash-safe execution journal, durable/sustained restart reconciliation, adapter integration, and residual-hedging policy. G4 now has live-override, simulation-mode, Keychain-source, production venue/chain, mode-coherence, and Kalshi-fragment hardening, but is not complete. G5 has verified per-order notional, single-venue pending-order exposure accounting, in-process open-order and distinct-position caps, and in-process rolling/daily placement-attempt caps, but remains open for cross-venue shared-ledger accounting and reconciliation-backed restart recovery.
