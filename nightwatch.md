...
| G4 | **PARTIAL** | `utils/config_loader.py:493-561` fails closed on live mode, simulation flags, production hosts/chains, credential fragments, and wallet-key source; `tests/test_config_loader.py` plus the redacted Keychain-backed probe verify the offline boundary. | Tracked-versus-ignored credential-source policy and a final conservative-default audit remain; Kalshi credential validity cannot be proved without prohibited authenticated access. |
...
| G10 | **PARTIAL** | `kalshi_client/models.py:52-125` correctly derives complete YES/NO ask depth from the opposite bid book. `kalshi_client/api.py` now admits only the current `orderbook_fp` schema, validates fixed-point strings, range, positive size, and strict ascending source order, and fails closed on deprecated, incomplete, or malformed depth; `tests/test_kalshi_client.py` covers derived depth, empty sides, and malformed inputs. This supersedes the source report's unsupported arbitrary 2% haircut. | The shared order-book model still converts authoritative fixed-point values to binary floats, and no bounded freshness gate prevents an old observation from reaching detection. Live depth quality is unproved. |
