...
from political_archive import select_archive_candidates
...
def _fetch_page(*, offset: int, limit: int) -> list[dict]:
    query = urlencode({
        "closed": "true", "tag_id": "2", "limit": str(limit), "offset": str(offset),
...
def main() -> int:
    ...
    if not 1 <= args.page_size <= 100 or args.max_events <= 0 or args.max_pages <= 0:
        parser.error("max-events/max-pages must be positive and page-size must be 1..100")
